...
class LeadsExportContact implements FromArray, WithHeadings
{

		use Exportable;

		public function __construct(array $contact_id)
		{
      // $this->ids = $lead_id;
...

		    {
				...
				$leads = Lead::with(['naics_codes','psc_codes','business_types','countries','states','cities','zips','mail_countries','mail_states','mail_cities','mailZips','physicalStreets','mailingStreets','stateOfIncorpo','countryOfIncorpo'])
	              ...
	              ->select('contacts.id as cid',
	               ...
	               'leads.*')
	                ->whereIn('contacts.id', $this->contact_id)->get();

					$contacts = [];
					// return response()->json($this->contact_id);
